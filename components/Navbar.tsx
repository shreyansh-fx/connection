...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
        ...
        (payload) => {
          ...
          setNotifications((previous) => {
            if (
              ...
              console.log(
                "[NAVBAR] Duplicate realtime notification ignored."
              );
      ...
      .subscribe((status) => {
        console.log(
          "[NAVBAR] Notification subscription status:",
          status
        );
      });
    ...
    return () => {
      ...
      console.log(
        "[NAVBAR] Notification channel removed."
      );
    };
  }, [supabase, user?.id]);
  ...
  const handleNotificationClick = async (
    ...
    if (
      ...
      console.log(
        "[NAVBAR] This is a request invitation."
      );
    ...
    if (!notification.related_id) {
      console.warn(
        "[NAVBAR] Notification has no related_id:",
        notification
      );
    ...
    const requestUrl =
      `/requests/${application.request_id}`;

    console.log(
      "[NAVBAR] Navigating to application request:",
      requestUrl
    );
                        ...
                        notifications.map(
                          (notification) => (
                            <button
                              ...
                              className={`w-full cursor-pointer rounded-xl border p-3 text-left transition hover:border-indigo-200 hover:bg-indigo-50/40 ${notification.is_read
